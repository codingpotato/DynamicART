...
typedef enum {
    CPLessArgument1,
    CPLessArgument2,
    CPLessNumberOfArguments
} CPLessArguments;

@implementation CPLess
...
- (CPBlockConfiguration *)createConfiguration {
    ...
    NSArray *defaultValueOfArguments = [NSArray arrayWithObjects:[CPNumberValue valueWithDouble:0.0], [CPNumberValue valueWithDouble:0], nil];
...
- (id<CPValue>)calculateResult {
    CPRightValueWeakTypeArgument *argument1 = [self.syntaxOrderArguments objectAtIndex:CPLessArgument1];
    CPRightValueWeakTypeArgument *argument2 = [self.syntaxOrderArguments objectAtIndex:CPLessArgument2];
    ...
    if ([result1 isKindOfClass:[CPStringValue class]]) {
        return [CPBooleanValue valueWithBoolean:[result1.stringValue compare:result2.stringValue] == NSOrderedAscending];
    } else {
